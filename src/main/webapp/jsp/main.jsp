...
<div class="container my-5">
    <c:choose>
    <c:when test="${cardsDrugList.isEmpty()}">
        <fmt:message key="main.errorMessage.noDrug"/>
    </c:when>
    <c:otherwise>
    <div class="row">
            ...
            <div class="col-sm-3">
                <form method="GET" action="controller">
                    ...
                    <div class="card  h-100" style="width: 20rem">
                        <c:if test="${!drug.drugPictureList.isEmpty()}">
                            <div id="drugCard${drugCounter.count}" class="carousel carousel-dark slide"
                                 data-bs-ride="carousel">
                                <div class="carousel-indicators">
                                    <c:forEach var="picture" items="${drug.drugPictureList}" varStatus="counter">
                                        <button type="button" data-bs-target="#drugCard${drugCounter.count}"
                                                data-bs-slide-to="${counter.count-1}"
                                                class="${counter.count==1?'active':''}"></button>
                                    </c:forEach>
                                </div>
                                ...
                                <button class="carousel-control-prev" type="button"
                                        data-bs-target="#drugCard${drugCounter.count}" data-bs-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                </button>
                                <button class="carousel-control-next" type="button"
                                        data-bs-target="#drugCard${drugCounter.count}" data-bs-slide="next">
                            ...
                            <c:choose>
                                <c:when test="${drug.needPrescription==true}">
                                    <a><fmt:message key="main.drugCard.footer.prescription"/>:<fmt:message
                                            key="main.drugCard.footer.prescription.true"/></a>
                                </c:when>
                                <c:otherwise>
                                    <a><fmt:message key="main.drugCard.footer.prescription"/>:<fmt:message
                                            key="main.drugCard.footer.prescription.false"/></a>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
...
<div class="container my-5">
    <form method="GET" action="controller" name="paginationForm">
        ...
        <nav>
            <ul class="pagination">
                <li class="page-item">
                    <button class="page-link" type="submit" name="paginationPage" value="previous"><fmt:message
                            key="main.pagination.button.previous"/></button>
                </li>
                <c:forEach var="number" begin="${startPaginationPage}" end="${lastPaginationPage}">
                    <li class="page-item  ${currentPaginationPage == number?'active':''}">
                        <button class="page-link" type="submit" name="paginationPage"
                                value="${number}">${number}</button>
                    </li>
                </c:forEach>
                <li class="page-item">
                    <button class="page-link" type="submit" name="paginationPage" value="next"><fmt:message
                            key="main.pagination.button.next"/></button>
                </li>
            </ul>
        </nav>
    </form>
